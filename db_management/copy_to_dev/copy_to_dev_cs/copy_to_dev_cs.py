...
from colabfit.tools.schema import configuration_set_schema, co_cs_map_schema
...
to_cs_table = "ndb.colabfit.dev.cs_tmp"
cs_table = "ndb.`colabfit-prod`.prod.cs"
...
_, bucket, schema, table_n = to_cs_table.split(".")
with sess.transaction() as tx:
    table = tx.bucket(bucket).schema(schema).table(table_n)
...
to_cs_map_table = "ndb.colabfit.dev.cs_co_tmp"
cs_map_table = "ndb.`colabfit-prod`.prod.cs_co_map_tmp"
...
cs_co_maps.write.mode("errorifexists").saveAsTable(
    to_cs_map_table, schema=co_cs_map_schema
)

# CS-CO map
sorted_columns = ["configuration_set_id", "configuration_id"]
unsorted_columns = []
_, bucket, schema, table_n = to_cs_map_table.split(".")
with sess.transaction() as tx:
    table = tx.bucket(bucket).schema(schema).table(table_n)
    table.create_projection(
