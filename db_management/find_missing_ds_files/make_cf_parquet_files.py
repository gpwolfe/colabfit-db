...
def get_cos(dataset_id, nconfigs):
    ...
    cos_save_path = Path(f"{dataset_id}/co.parquet")
    n_partitions = max(1, nconfigs // 100000)
    cos.repartition(n_partitions).write.parquet(str(cos_save_path))
...
def get_pos(dataset_id, nconfigs):
    ...
    pos_save_path = Path(f"{dataset_id}/po.parquet")
    n_partitions = max(1, nconfigs // 100000)
...
def get_dataset(dataset_id):
    ...
    ds.write.json(f"{dataset_id}/ds.json")
    logger.info(f"Saved DS for {dataset_id} to {dataset_id}/ds.json")
...
def get_configuration_sets(dataset_id):
    ...
    cs.write.parquet(f"{dataset_id}/cs.parquet")
    logger.info(f"Saved CS for {dataset_id} to {dataset_id}/cs.parquet")
    return cs.select("id")
...
def get_cs_co_mapping(dataset_id, ids):
    ...
    cs_co_map.write.parquet(f"{dataset_id}/cs_co_map.parquet")
    logger.info(
        f"Saved CS-CO mapping for {dataset_id} to {dataset_id}/cs_co_map.parquet"
    )
...
def process_dataset(dataset_id):
    ...
    if Path(dataset_id).exists():
        logger.info(f"Dataset {dataset_id} already processed, skipping.")
        return
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        logger.error("Usage: python make_cf_parquet_files.py <dataset_ids_file>")
        sys.exit(1)
