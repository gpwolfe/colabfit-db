...
def export_configuration_parquets(dataset_id, dataset_dir, session, table_suffix):
...
def export_configurations_in_batches(dataset_id, dataset_dir, session, table_suffix):
...
def export_configuration_sets(dataset_id, dataset_dir, session, table_suffix):
...
def export_cs_co_mapping(cs_ids_all, dataset_dir, session, table_suffix):
    if not cs_ids_all:
        return
...
def get_dataset_data(dataset_id, session, table_suffix):
    with session.transaction() as tx:
...
def process_dataset(dataset_id, table_suffix):
    ...
    try:
        ...
        ds_data = get_dataset_data(dataset_id, session, table_suffix)
        ...
        if nconfigs > CONFIG["LARGE_DATASET_THRESHOLD"]:
            ...
            export_configurations_in_batches(
                dataset_id, dataset_dir, session, table_suffix
            )
        ...
        if check_table_exists(session, f"cs_{table_suffix}"):
            ...
            logger.info(f"Table cs_{table_suffix} does not exist, skipping CS export")

        if cs_ids_all and check_table_exists(session, f"cs_co_map_{table_suffix}"):
            export_cs_co_mapping(cs_ids_all, dataset_dir, session, table_suffix)
        elif cs_ids_all:
            logger.info(
